...
export default function ServiceDetailPage() {
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-20">
      ...
      <div className="sticky top-20 z-40 bg-white/95 backdrop-blur shadow-md border-y border-slate-200 transition-all">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <div className="flex items-center justify-center gap-2 sm:gap-6 py-3">
            {/* Tab 1: Về chúng tôi */}
            <button
              onClick={() => scrollToSection("about")}
              className={`flex items-center gap-2 px-4 sm:px-6 py-2.5 rounded-xl font-bold text-xs sm:text-sm uppercase tracking-wide transition-all ${
                activeTab === "about"
                  ? "bg-[#641D06] text-white shadow-md border border-[#641D06]"
      ...
      {/* Main Content Area */}
      <main className="max-w-7xl mx-auto px-4 md:px-8 pt-10 space-y-16">
        {/* SECTION 1: VỀ CHÚNG TÔI (ABOUT) */}
        ...
        {/* SECTION 2: KINH NGHIỆM THỰC CHIẾN (EXPERIENCE) */}
        <section id="section-experience" className="space-y-8 pt-8">
          <div className="text-center max-w-2xl mx-auto">
            ...
            <p className="text-slate-600 text-sm mt-2">
              Những thương vụ tư vấn, giải quyết tranh chấp và hỗ trợ pháp lý thành công của Đức Tín &amp; Cộng sự.
            </p>
          </div>

          <div className="space-y-6">
            {service.experience.map((exp, idx) => (
              <div key={idx} className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col md:flex-row gap-6">
                <div className="md:w-1/4 shrink-0 border-b md:border-b-0 md:border-r border-slate-100 pb-4 md:pb-0 md:pr-6 flex flex-col justify-between">
                  <div>
                    <span className="px-3 py-1 rounded-full bg-slate-100 text-slate-700 text-xs font-bold uppercase">
                      {exp.year}
                    </span>
                    <h3 className="font-bold text-slate-900 text-base mt-3">{exp.clientType}</h3>
                  </div>
                  <div className="text-emerald-700 font-bold text-xs uppercase tracking-wider mt-4 flex items-center gap-1">
                    <span className="material-symbols-outlined text-base">check</span>
                    Đã hoàn tất thắng lợi
                  </div>
                </div>

                <div className="md:w-3/4 space-y-3">
                  <h3 className="text-xl font-bold text-[#641D06]">
                    {exp.title}
                  </h3>
                  <div className="bg-emerald-50/80 p-4 rounded-xl border border-emerald-100 text-emerald-900 text-sm font-semibold">
                    🎯 Kết quả: {exp.result}
                  </div>
        ...
        {/* SECTION 3: BÀI VIẾT CHUYÊN MÔN (ARTICLES) */}
        <section id="section-articles" className="space-y-8 pt-8">
          <div className="text-center max-w-2xl mx-auto">
            ...
            <p className="text-slate-600 text-sm mt-2">
              Các bài phân tích chuyên sâu bởi đội ngũ Luật sư Đức Tín &amp; Cộng sự.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {service.articles.map((art, idx) => (
              <div key={idx} className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm flex flex-col justify-between hover:border-[#641D06] transition-colors">
                <div>
                  <div className="flex items-center justify-between text-xs text-slate-500 font-semibold mb-3">
                    <span>📅 {art.date}</span>
                    <span>⏱️ {art.readTime}</span>
                  </div>
                  <h3 className="text-xl font-bold text-slate-900 mb-3 hover:text-amber-700 transition-colors">
                ...
                <Link
                  href={`/news/${art.slug}`}
                  className="inline-flex items-center gap-2 text-[#641D06] font-bold text-sm uppercase hover:text-amber-700 transition-colors"
                >
                  Đọc toàn bộ bài viết <span className="material-symbols-outlined text-sm">arrow_forward</span>
                </Link>
              </div>
            ))}
          </div>
        ...
        <section className="bg-gradient-to-r from-[#641D06] to-[#3B1104] text-white p-8 md:p-14 rounded-3xl shadow-2xl text-center">
