...
export default function FlowchartVisual({ rawText }: { rawText?: string }) {
  ...
  const areAllExpanded = data.phases.every((p) => expandedPhases[p.phaseNumber]);

  const toggleAll = () => {
    const nextState = !areAllExpanded;
    const newExpanded: Record<number, boolean> = {};
    data.phases.forEach((p) => {
      newExpanded[p.phaseNumber] = nextState;
    });
  ...
  return (
    <div className="my-6 w-full rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 shadow-2xs">
        ...
        <div className="flex items-center gap-2">
          {isOpen && (
            <button
              type="button"
              onClick={toggleAll}
              className="text-[11px] font-bold text-[#641D06] bg-amber-50 hover:bg-amber-100 border border-amber-200 px-2.5 py-1.5 rounded-xl transition-colors cursor-pointer"
            >
              {areAllExpanded ? "Thu gọn tất cả" : "Xổ tất cả bước"}
            </button>
          )}
          ...
          <button
            ...
            className="flex items-center gap-1 text-[11px] font-bold text-slate-700 bg-slate-50 hover:bg-slate-100 border border-slate-200 px-2.5 py-1.5 rounded-xl transition-colors cursor-pointer"
          >
            <span className="material-symbols-outlined text-sm">
              {isOpen ? "visibility_off" : "visibility"}
            </span>
            <span>{isOpen ? "Ẩn" : "Xem"}</span>
          </button>
        </div>
      </div>
      ...
      {/* SƠ ĐỒ TREE DIAGRAM: BAN ĐẦU CHỈ HIỆN GIAI ĐOẠN, ẤN VÀO XỔ RA CÁC BƯỚC */}
      {isOpen && (
        <div className="w-full flex flex-col items-center space-y-3">
          {data.phases.map((phaseRow, idx) => {
            const isLast = idx === data.phases.length - 1;
            const subCount = phaseRow.subSteps.length;
            const isExpanded = !!expandedPhases[phaseRow.phaseNumber];

            return (
              <React.Fragment key={idx}>
                {/* KHUNG BỌC GIAI ĐOẠN */}
                <div
                  className={`w-full rounded-2xl p-3 sm:p-4 flex flex-col items-center transition-all ${phaseRow.theme.outerBorder} ${phaseRow.theme.outerBg}`}
                >
                  {/* THANH HEADER TIÊU ĐỀ GIAI ĐOẠN (CLICK ĐỂ XỔ RA / THU GỌN) */}
                  <button
                    type="button"
                    onClick={() => togglePhase(phaseRow.phaseNumber)}
                    className={`w-full flex items-center justify-between px-3.5 py-3 rounded-xl bg-white border border-slate-200/80 shadow-2xs ${phaseRow.theme.borderLeft} cursor-pointer hover:shadow-xs transition-all active:scale-[0.99] text-left group`}
                  >
                    <div className="flex items-center gap-2.5 min-w-0 pr-2">
                      <span
                        className={`px-2.5 py-0.5 rounded-md text-[10.5px] uppercase tracking-wider shrink-0 shadow-2xs ${phaseRow.theme.badgeBg}`}
                      >
                        {phaseRow.label}
                      </span>
                      <h5 className="text-xs sm:text-sm font-extrabold text-slate-900 leading-snug group-hover:text-[#641D06] transition-colors">
                        {phaseRow.mainTitle}
                      </h5>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {subCount > 0 && (
                        ...
                        >
                          <span>{subCount} bước</span>
                        </span>
                      )}
                      <span className="material-symbols-outlined text-base text-slate-400 group-hover:text-slate-700 transition-transform duration-200">
                        {isExpanded ? "expand_less" : "expand_more"}
                      </span>
                    </div>
                  </button>

                  {/* CÁC BƯỚC PHỤ PHÂN NHÁNH CÂY (HIỆN RA KHI ĐƯỢC XỔ RA) */}
                  {isExpanded && subCount > 0 && (
                    <div className="w-full flex flex-col items-center mt-2 animate-fadeIn">
                ...
                {/* MŨI TÊN DỌC CHỈA XUỐNG SANG GIAI ĐOẠN TIẾP THEO */}
                {!isLast && (
                  <div className="flex flex-col items-center justify-center my-0.5 select-none">
                    <div className="w-0.5 h-2 bg-slate-300" />
                    <div className="w-5 h-5 rounded-full bg-white border border-slate-300 flex items-center justify-center shadow-2xs -my-0.5 z-10">
                      <span className="material-symbols-outlined text-[10px] font-black text-[#641D06]">
                        arrow_downward
                      </span>
                    </div>
                    <div className="w-0.5 h-2 bg-slate-300" />
          ...
          <div className="w-full flex items-center justify-between text-[10px] text-slate-500 pt-3 mt-3 border-t border-slate-100">
